...
library(reticulate)
################INITIAL SETUP: run these to install all needed python libraries into reticulates default environment
#py_install("fuzzywuzzy")
#py_install("tqdm")
py_install("lxml")
py_install("RISparser",pip = TRUE)
####################################################################
...
# # todo reticulate needs python libraries
# 
...
# 
# # Update Arxiv and DBLP and ACL

#reticulate::import_from_path("SystematicSearcher", path = "./data/")
reticulate::import_from_path("scrape", path = "./data/")


#reticulate::source_python("data/scrape.py")
#reticulate::py_run_file("data/run_search.py")
...
#
#arXiv data
arx_data <- read.csv("data/results_arxiv.csv", stringsAsFactors = FALSE, encoding = "UTF-8", header = TRUE)

# # Clean results
arx_clean_results <- data.frame(stringsAsFactors = FALSE,
                                 title       = arx_data$Title,
                                 abstract    = arx_data$Abstract,
                                 authors     = arx_data$Authors,
                                 link        = arx_data$Url,
                                 date        = arx_data$Created,
                                 subject     = arx_data$Categories)


if (nrow(arx_clean_results) == 0){
  arx_clean_results$source = numeric(nrow(arx_clean_results))
  
} else {
  arx_clean_results$source <- 'arXiv'
  
}

##########################################
#aCL
acl_data <- read.csv("data/results_acl.csv", stringsAsFactors = FALSE, encoding = "UTF-8", header = TRUE)

# # Clean results
acl_clean_results <- data.frame(stringsAsFactors = FALSE,
                                title       = acl_data$Title,
                                abstract    = acl_data$Abstract,
                                authors     = acl_data$Authors,
                                link        = acl_data$Url,
                                date        = acl_data$Year,
                                subject     = acl_data$DOI_link)


if (nrow(acl_clean_results) == 0){
  acl_clean_results$source = numeric(nrow(acl_clean_results))
  
} else {
  acl_clean_results$source <- 'ACL'
...
#
#arXiv data
dblp_data <- read.csv("data/results_dblp.csv", stringsAsFactors = FALSE, encoding = "UTF-8", header = TRUE)

# # Clean results
dblp_clean_results <- data.frame(stringsAsFactors = FALSE,
                                title       = dblp_data$Title,
                                authors     = dblp_data$Authors,
                                link        = dblp_data$DOI_link,
                                date        = dblp_data$Date)
if (nrow(dblp_clean_results) == 0){
dblp_clean_results$source = numeric(nrow(dblp_clean_results))
dblp_clean_results$subject = numeric(nrow(dblp_clean_results))
dblp_clean_results$abstract = numeric(nrow(dblp_clean_results))
} else {
  dblp_clean_results$source <- 'DBLP'
  dblp_clean_results$subject <- 'NA'
...
# }
# 
# scop_results <- perform_search(regex_query, scop_data, fields = c("title","abstract"))# error #Error in mx_data %>% dplyr::filter_at(dplyr::vars(fields), dplyr::any_vars(grepl(paste(query[[list]],  :  could not find function "%>%"
# 
# # Clean results
...
####adding manual data
#########################################################all_results <- read.csv("data/extension.csv", stringsAsFactors = FALSE, 
                                            #encoding = "UTF-8", header = TRUE)
###################################################colnames(all_results)[1] <- "title" 
###########################names(all_results)
...
##########################################################################normally, use deduped above only
##new_results <- read.csv("data/results/new_results.csv",
##                        stringsAsFactors = FALSE)  %>%
##  mutate_all(replace_na, replace = "") %>%
##  distinct(link, .keep_all = TRUE)

###DANGER REPLACE WITH ACTUAL ALL RESULTSprevious_results <- read.csv("data/screening_snapshot/2020-09-24_1_snapshot.csv",
                             #stringsAsFactors = FALSE) 
# 
...
#print(names(previous_results))
#print(names(new_results))
...
#Set-up
  ...
  #########################################SNAPSHOT REVIEWER 1
  db_snapshot_name1 <- paste0("data/screening_snapshot/",current_date,"_1_snapshot.csv")
...
# Take and save snapshot of the database, and add new results
  ...
  write.csv(db_snapshot,
            file = db_snapshot_name1,
            fileEncoding = "UTF-8",
            row.names = FALSE)
 db$insert(new_results)################UNCOMMENT AT FIRST UPDATE
  #########################################SNAPSHOT REVIEWER 1
  db_snapshot_name2 <- paste0("data/screening_snapshot/",current_date,"_2_snapshot.csv")
  
  
  # Take and save snapshot of the database, and add new results
  databaseName <- "LSR"
  collectionName <- "reviewer_2"
  mongo_url <- paste0("mongodb+srv://LS:",
                      readLines("app/password.txt"),
                      "@lsr.7ybnv.mongodb.net/LSR?retryWrites=true&w=majority")
  db <- mongo(collection = collectionName,
...
# Add and commit files
  ...
  add(repo = getwd(),
      path = db_snapshot_name1)
  
  add(repo = getwd(),
      path = db_snapshot_name2)
...
# Push the repo again
  push(object = getwd(), "origin", "refs/heads/master",
